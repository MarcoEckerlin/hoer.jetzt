...
public class AccessGuard {
    ...
    public Optional<BotAdminRole> botAdminRole(DashboardSession session) {
        ...
        notfalltuerPruefen(session);

        return botAdminService.roleOf(session.userId());
    }

    /**
     * Die Notfalltuer: {@code HJ_BOT_ADMIN_IDS} in der Umgebung.
     *
     * <p>Normalerweise uebernimmt sich der Eigentuemer der Discord-Anwendung
     * beim ersten Aufruf selbst. Das deckt den Normalfall ab und ist bequem -
     * aber es deckt nicht alle:</p>
     *
     * <ul>
     *   <li>Die Anwendung gehoert einem <b>Team</b>. Dann zaehlt nur der
     *       Team-Eigentuemer, nicht die uebrigen Mitglieder - und wer den
     *       Zugang zu diesem Konto nicht hat, kommt nicht hinein.</li>
     *   <li>Discord antwortet gerade nicht, und die Uebernahme scheitert
     *       stillschweigend.</li>
     * </ul>
     *
     * <p>Ohne diese Tuer bliebe nur der Weg ueber die Datenbank - also genau
     * das, was man nachts um drei nicht tun will.</p>
     *
     * <p>Die Kennungen stehen in einer Umgebungsvariablen, nicht in der
     * Datenbank und nicht in der Oberflaeche: wer sie setzen kann, hat ohnehin
     * Zugriff auf den Server. Eingetragen wird erst, wenn sich die Person
     * tatsaechlich anmeldet - eine Kennung in einer Datei ist eine Absicht,
     * kein Konto.</p>
     */
    private void notfalltuerPruefen(DashboardSession session) {
        String liste = System.getenv("HJ_BOT_ADMIN_IDS");
        if (liste == null || liste.isBlank()) {
            return;
        }
        String meine = session.userId().trim();
        for (String eintrag : liste.split("[,;\\s]+")) {
            if (!eintrag.trim().equals(meine)) {
                continue;
            }
            if (botAdminService.roleOf(meine).isPresent()) {
                return;
            }
            try {
                botAdminService.notfallOwner(meine, session.username());
                Alert.send("INFO", "SECURITY", "Bot-Admin aus HJ_BOT_ADMIN_IDS uebernommen: " + meine);
            } catch (Exception fehler) {
                Alert.send("WARN", "SECURITY",
                        "Bot-Admin aus HJ_BOT_ADMIN_IDS konnte nicht eingetragen werden: " + fehler.getMessage());
            }
            return;
        }
    }
