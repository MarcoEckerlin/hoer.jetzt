...
public class GuildAudioState {
    ...
    /**
     * Die zuletzt gesicherte Abspielposition und wann sie galt.
     *
     * <p>Reisst die Sprachverbindung ab, meldet der Player oft 0 oder gar
     * nichts mehr - der eigentliche Wert ist mit der Verbindung weg. Wer beim
     * Wiederverbinden diese 0 benutzt, faengt den Titel von vorne an. Genau
     * das war der Fehler "Titel werden nach einer Zeit zurueckgesetzt".</p>
     */
    private long letztePositionMs;
    private long letztePositionZeit;
    /** Laeuft gerade ein automatischer Reconnect? Verhindert Mehrfachstarts. */
    private boolean reconnectLaeuft;

    public synchronized void merkePosition(long positionMs) {
        if (positionMs > 0) {
            this.letztePositionMs = positionMs;
            this.letztePositionZeit = System.currentTimeMillis();
        }
    }

    /**
     * Die letzte bekannte Position, um die verstrichene Zeit fortgeschrieben.
     *
     * <p>Zwischen dem letzten Merken und dem Abriss vergehen bis zu zehn
     * Sekunden. Die einfach zu verschenken hiesse, nach jedem Abriss ein
     * Stueck doppelt zu hoeren.</p>
     *
     * @param maxAlterMs aelter als das - dann ist der Wert wertlos
     */
    public synchronized long letztePosition(long maxAlterMs) {
        if (letztePositionMs <= 0 || letztePositionZeit <= 0) {
            return 0L;
        }
        long alter = System.currentTimeMillis() - letztePositionZeit;
        if (alter < 0 || alter > maxAlterMs) {
            return 0L;
        }
        return letztePositionMs + alter;
    }

    public synchronized void vergissPosition() {
        letztePositionMs = 0L;
        letztePositionZeit = 0L;
    }

    public synchronized boolean beginneReconnect() {
        if (reconnectLaeuft) {
            return false;
        }
        reconnectLaeuft = true;
        return true;
    }

    public synchronized void beendeReconnect() {
        reconnectLaeuft = false;
    }
