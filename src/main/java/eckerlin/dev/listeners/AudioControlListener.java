...
public class AudioControlListener extends ListenerAdapter {
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        if (parts.length != 3) {
            event.reply("Die Audio-Steuerung ist ungültig.").setEphemeral(true).queue();
            return;
        }
        ...
        if (!canControl(member, guild)) {
            event.reply("Du musst im gleichen Voice-Channel sein oder den Server verwalten dürfen.").setEphemeral(true).queue();
            return;
        }
