...
public class RadioCommand implements SlashCommand {
    ...
    public List<Command.Choice> complete(CommandAutoCompleteInteractionEvent event) {
        ...
        return audioService.getStations().stream()
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        if (radioId == null || radioId < 1) {
            CommandHelper.replyError(
                    event,
                    "Radio",
                    "Bitte waehle einen Radiosender oder gib eine gueltige Radio-ID an. Mit `/radios` kannst du dir die Senderliste anzeigen lassen.",
                    true
            );
            return;
        }
