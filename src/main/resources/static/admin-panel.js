...
(function () {
    ...
    var PAGES = {
        ...
        lavalink:    { icon: "#i-plug",     title: "Audio-Knoten" },
    ...
    /** Eingabefeld mit Beschriftung darueber - in Wiederholungslisten Pflicht. */
    function beschriftet(text, feld) {
        return '<label class="beschriftet"><span>' + esc(text) + '</span>' + feld + '</label>';
    }

    function renderLavalink() {
        var list = (changed.lavalinkNodes || (state.config && state.config.lavalinkNodes) || []);
        var html = head("lavalink",
            "Einrichtung und Auslastung. Der Bot verteilt Server auf den am wenigsten belasteten Knoten der passenden Stufe und zieht bei Ausfall selbsttätig um.",
            roleAtLeast("ADMIN")
                ? '<button class="btn btn-secondary btn-sm" data-action="reload-audio-nodes">Knoten jetzt einlesen</button>'
                  + '<button class="btn btn-secondary btn-sm" data-action="rebalance-audio">Server neu verteilen</button>'
                : "");
        ...
        if (!list.length) {
            ...
            list.forEach(function (entry, index) {
                html += '<div class="repeat-item" data-list="lavalinkNodes" data-index="' + index + '">'
                    + '<span class="grip"><svg viewBox="0 0 24 24" width="18" height="18"><use href="#i-grip"/></svg></span>'
                    + '<span class="fields">'
                    + beschriftet("Name", '<input class="input" data-field="nodeName" placeholder="z. B. premium-1" value="' + esc(entry.nodeName) + '">')
                    + beschriftet("Deployment-Schlüssel (leer = aktuelles)", '<input class="input mono" data-field="deploymentKey" placeholder="local" value="' + esc(entry.deploymentKey) + '">')
                    + beschriftet("Adresse", '<input class="input mono" data-field="serverUri" placeholder="http://127.0.0.1:2333" value="' + esc(entry.serverUri) + '">')
                    + beschriftet("Passwort", '<input class="input mono" type="password" data-field="password" placeholder="leer = unverändert" value="">')
                    + beschriftet("Zeitlimit (ms)", '<input class="input" type="number" data-field="httpTimeoutMs" title="So lange wartet der Bot auf Antwort, bevor er den Knoten als ausgefallen behandelt." value="' + esc(entry.httpTimeoutMs) + '">')
                    + beschriftet("Wiederaufnahme (s)", '<input class="input" type="number" data-field="resumeTimeoutSeconds" title="So lange haelt Lavalink die Wiedergabe, wenn der Bot verschwindet - das ueberbrueckt Neustarts." value="' + esc(entry.resumeTimeoutSeconds) + '">')
                    + beschriftet("Stufe", '<select class="input" data-field="tier">'
                        + '<option value="free"' + (entry.tier === "premium" ? "" : " selected") + '>Standard</option>'
                        + '<option value="premium"' + (entry.tier === "premium" ? " selected" : "") + '>Premium</option>'
                        + '</select>')
                    + beschriftet("Obergrenze", '<input class="input" type="number" min="0" data-field="maxPlayers" title="Gleichzeitige Wiedergaben. 0 = unbegrenzt." value="' + esc(entry.maxPlayers == null ? 0 : entry.maxPlayers) + '">')
                    + '<label class="row" style="padding:0;border:0;gap:8px;align-items:center">'
                    + '<span class="switch"><input type="checkbox" data-field="enabled"' + (entry.enabled ? " checked" : "")
                    + '><span class="track"></span><span class="knob"></span></span>'
                    + '<span style="font-size:14px;color:var(--text-muted)">aktiv</span></label>'
        ...
        html += '<button class="btn btn-secondary btn-sm" style="margin-top:12px" data-add="lavalinkNodes">'
            ...
            + '<div class="hint" style="margin-top:6px"><b>Last</b> ist die Bewertung, nach der der Bot entscheidet, wohin ein neuer Server geht — je höher, desto belasteter. <b>Liegt falsch</b> heisst: der Server gehört auf eine andere Stufe, meist weil er während eines Ausfalls ausgewichen ist.</div>'
            + '<div class="hint" style="margin-top:6px">Auf <b>Premium</b>-Knoten landen nur Server, für die <b>Premium-Audio</b> freigeschaltet ist — die Freigabe steht unter <i>Server</i>. Ist kein Premium-Knoten erreichbar, weicht der Bot auf Standard aus und schreibt es ins Protokoll. Die <b>Obergrenze</b> ist die Notbremse: der Bot weicht schon vorher aus, wenn ein Knoten unter Last gerät.</div>'
            + '<div class="hint" style="margin-top:6px">Umgekehrt gilt dasselbe: sind alle <b>Standard</b>-Knoten voll, dürfen Standard-Server als <b>Überlauf</b> auf einen Premium-Knoten — sonst bliebe der nächste Server stumm, während nebenan Kapazität frei steht. Premium behält Vorrang: der Überlauf greift nur auf Knoten mit Luft, und ein Platz bleibt reserviert. Sobald wieder ein Standard-Knoten frei ist, ziehen sie von selbst zurück. Abschalten mit <code>HJ_LAVALINK_FREE_OVERFLOW=false</code>.</div>'
            + '</div></div>';
        return html;
    }
    ...
    function dauer(sekunden) {
        if (!sekunden) { return "\u2013"; }
        var s = Math.floor(sekunden), tage = Math.floor(s / 86400), stunden = Math.floor((s % 86400) / 3600);
        if (tage > 0) { return tage + " d " + stunden + " h"; }
        var minuten = Math.floor((s % 3600) / 60);
        return stunden > 0 ? stunden + " h " + minuten + " min" : minuten + " min";
    }

    /**
     * Lebendzustand eines Knotens, direkt ueber seinen Einstellungen.
     *
     * <p>Getrennte Seiten fuer "einrichten" und "nachsehen" bedeuteten, beim
     * Suchen eines Problems staendig zu wechseln. Hier steht beides
     * uebereinander: was eingestellt ist, und was daraus geworden ist.
     */
    function knotenZustand(name) {
        var knoten = (state.audioNodes || []).filter(function (n) { return n.name === name; })[0];
        if (!knoten) {
            return '<div class="hint" style="margin-top:8px">Nicht angemeldet — '
                + 'nach dem Speichern übernimmt der Bot ihn von selbst.</div>';
        }

        var last = Math.round((knoten.cpuLast || 0) * 100);
        var voll = knoten.obergrenze > 0 && knoten.spielend >= knoten.obergrenze;

        var html = '<div class="row" style="margin-top:8px"><div class="label"><b>Zustand</b><span>'
            + (knoten.erreichbar
                ? knoten.spielend + ' Wiedergaben aktiv von ' + knoten.gesamt + ' Playern'
                    + (knoten.obergrenze > 0 ? ' · Obergrenze ' + knoten.obergrenze : '')
                    + (voll ? ' · <b>Obergrenze erreicht</b>' : '')
                    + ' · CPU ' + last + ' % · Last ' + knoten.strafpunkte
                : 'antwortet nicht')
            + '</span></div><div class="control">'
            + (knoten.erreichbar ? '<span class="badge success">läuft</span>'
                                 : '<span class="badge danger">weg</span>')
            + '</div></div>';

        if (knoten.server && knoten.server.length) {
            knoten.server.forEach(function (g) {
                // Ein Standard-Server auf einem Premium-Knoten liegt nicht
                // falsch, sondern im Überlauf: die Standard-Knoten waren voll.
                // Er zieht von selbst zurück, sobald dort wieder Platz ist.
                var ueberlauf = !g.passtZurStufe && g.stufe === "free" && knoten.stufe === "premium";
                var hinweis = g.passtZurStufe ? ""
                    : (ueberlauf ? ' · <b>Überlauf</b>' : ' · <b>liegt falsch</b>');
                var abzeichen = g.passtZurStufe ? ""
                    : (ueberlauf ? '<span class="badge">Überlauf</span>'
                                 : '<span class="badge danger">Umzug fällig</span>');
                html += '<div class="row"><div class="label"><b>' + esc(g.name) + '</b><span>'
                    + (g.spielt ? esc(g.titel || "spielt") : "verbunden, still")
                    + ' · Stufe ' + (g.stufe === "premium" ? "Premium" : "Standard")
                    + hinweis
                    + '</span></div><div class="control">' + abzeichen + '</div></div>';
            });
        } else if (knoten.erreichbar) {
            html += '<div class="hint" style="margin-top:6px">Kein Server auf diesem Knoten.</div>';
        }

        if (roleAtLeast("ADMIN")) {
            html += '<div class="row"><div class="label"><b>Neu verbinden</b><span>'
                + 'Trennt die Verbindung zu diesem Knoten und baut sie sofort wieder auf. '
                + 'Hilft gegen eine hängende Sitzung. Laufende Server ziehen kurz um und kommen zurück. '
                + 'Der Lavalink-Dienst selbst wird dabei <b>nicht</b> neu gestartet — das geht nur auf dem Host.'
                + '</span></div><div class="control">'
                + '<button class="btn btn-secondary btn-sm" data-node-reconnect="' + esc(name) + '">Neu verbinden</button>'
                + '</div></div>';
        }
        return html;
    }
    ...
    function renderGuilds() {
        ...
        guilds.forEach(function (guild) {
            ...
            entitlements.forEach(function (entry) {
                // Premium-Audio ist eine Zuteilung, kein Kontingent: der Server
                // landet auf den Premium-Knoten oder eben nicht. Ein Tageslimit
                // waere hier sinnlos und wird deshalb gar nicht erst angeboten.
                var istPremium = entry.feature === "PREMIUM_AUDIO";
                var zustand = entry.enabled
                    ? (istPremium
                        ? ('zugeteilt' + (entry.grantedBy ? ' von ' + esc(entry.grantedBy) : '') + ' · läuft auf den Premium-Knoten')
                        : ('freigeschaltet' + (entry.grantedBy ? ' von ' + esc(entry.grantedBy) : '')
                           + (entry.dailyLimit > 0
                                ? ' · heute ' + entry.usedToday + ' von ' + entry.dailyLimit + ' Aufrufen'
                                : ' · ohne Tageslimit')))
                    : (istPremium ? 'Standard-Knoten' : 'gesperrt');

                html += '<div class="row' + (istPremium ? ' row-premium' : '') + '"><div class="label"><b>' + esc(entry.featureLabel) + '</b>'
                    + '<span>' + zustand + '</span></div>'
                    + '<div class="control">'
                    + (istPremium ? '' : ('<input class="input" type="number" min="0" style="width:110px" title="Tageslimit, 0 = unbegrenzt"'
    ...
    async function save() {
        ...
        try {
            ...
            toast("Gespeichert.");
            await loadConfig();
            // Der Bot uebernimmt neue Knoten beim Speichern - der Zustand
            // daneben soll das sofort zeigen, nicht erst beim naechsten Laden.
            state.audioNodes = await api("GET", "/api/admin/audio/nodes").catch(function () { return []; });
            show(state.page);
        } catch (error) {
    ...
    async function loadAll() {
        try {
            var results = await Promise.all([
                ...
                // Faellt einzeln aus, wenn der Bot noch nicht verbunden ist -
                // deswegen soll nicht die ganze Verwaltung leer bleiben.
                api("GET", "/api/admin/audio/nodes").catch(function () { return []; })
            ]);
    ...
    page.addEventListener("click", async function (event) {
        ...
        if (add) {
            ...
            // Vorbelegen statt leer lassen: beide Felder sind Pflicht, und der
            // Schluessel des laufenden Deployments ist in aller Regel der
            // richtige. Ein leeres Feld waere nur eine Fehlermeldung in spe.
            var schluessel = (state.config && state.config.currentDeploymentKey) || "local";
            var laufendeNummer = list.length + 1;
            list.push(name === "deployments"
                ? { deploymentKey: "", displayName: "", baseUrl: "", redirectUri: "", webPort: null, enabled: true, sortOrder: list.length }
                : { nodeName: "knoten-" + laufendeNummer, deploymentKey: schluessel, serverUri: "http://127.0.0.1:2333", password: null, httpTimeoutMs: 15000, resumeEnabled: true, resumeTimeoutSeconds: 180, enabled: true, tier: "free", maxPlayers: 0 });
        ...
        var reconnect = target.closest("[data-node-reconnect]");
        if (reconnect) {
            var knotenName = reconnect.dataset.nodeReconnect;
            if (!window.confirm('Knoten „' + knotenName + '" neu verbinden? '
                    + 'Laufende Wiedergaben auf diesem Knoten setzen dabei kurz aus.')) {
                return;
            }
            reconnect.disabled = true;
            try {
                var antwortKnoten = await api("POST",
                    "/api/admin/audio/nodes/" + encodeURIComponent(knotenName) + "/reconnect");
                toast(antwortKnoten.message);
                state.audioNodes = await api("GET", "/api/admin/audio/nodes");
                show(state.page);
            } catch (error) {
                toast(error.message, true);
            } finally {
    ...
    async function runAction(name, element) {
        // Der Bot gleicht die Knotentabelle ohnehin von selbst ab. Dieser Knopf
        // ist für den Moment, in dem man nicht warten will - etwa direkt nach
        // dem Einrichten eines neuen Hosts.
        if (name === "reload-audio-nodes") {
            element.disabled = true;
            try {
                var geladen = await api("POST", "/api/admin/actions/reload-audio-nodes");
                toast(geladen.message, geladen.success === false);
                state.audioNodes = await api("GET", "/api/admin/audio/nodes");
                show(state.page);
            } catch (error) {
                toast(error.message, true);
        ...
        if (name === "rebalance-audio") {
            element.disabled = true;
            try {
                var antwort = await api("POST", "/api/admin/actions/rebalance-audio");
