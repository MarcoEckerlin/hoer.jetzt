...
(function () {
    ...
    var pollTimer = null;
    var vorherigeSeite = null;   // fuer die Unterscheidung Wechsel/Auffrischung
    var adresseSelbstGesetzt = false;  // unterscheidet eigene Adressaenderung von Zurueck
    var neuaufbauOffen = false;  // Auffrischung, die wegen einer Eingabe wartet

    // Felder, die nur im Browser leben: der Server kennt sie nicht, ein
    // Neuaufbau der Seite wuerde sie also kommentarlos leeren. Genau das
    // passierte alle fuenf Sekunden, waehrend man den Suchbegriff tippte.
    var FLUECHTIG = ["playQuery", "playChannel", "radioChannel"];
    ...
    var PAGES = {
        ...
        invitelink:    { icon: "#i-link",   title: "Einladungslink" },
        vorlagen:      { icon: "#i-brand",  title: "Nachrichten-Vorlagen" },
    ...
    function markDirty() {
        // Die gemeinsame Speicherleiste gehoert den Modulen. Seiten mit
        // eigenem Speicherknopf (Vorlagen, Einladungslink) haben kein Modul -
        // dort wuerde sie in einen Endpunkt "undefined" laufen.
    ...
    function renderPlayer() {
        ...
        // Nur fuer das Team: der Server liefert die Angaben gar nicht erst aus,
        // wenn das Recht fehlt - dann bleibt state.stream leer.
        var technik = state.stream;
        if (technik && technik.knoten) {
            html += '<div class="card"><h2>Technik'
                + '<span class="sub">Nur für das Server-Team sichtbar</span></h2><div class="card-body">'
                + '<div class="row"><div class="label"><b>Audio-Knoten</b><span>'
                + esc(technik.knoten) + ' · Stufe ' + esc(technik.knotenStufe)
                + (technik.knotenErreichbar ? '' : ' · <b>nicht erreichbar</b>')
                + '</span></div><div class="control">'
                + (technik.knotenErreichbar ? '<span class="badge success">läuft</span>'
                                            : '<span class="badge danger">weg</span>')
                + '</div></div>'
                + '<div class="row"><div class="label"><b>Auslastung des Knotens</b><span>'
                + technik.wiedergabenAufKnoten + ' Wiedergaben gleichzeitig</span></div>'
                + '<div class="control">' + technik.wiedergabenAufKnoten + '</div></div>'
                + '<div class="row"><div class="label"><b>Stufe dieses Servers</b><span>'
                + esc(technik.serverStufe)
                + (technik.passtZurStufe ? '' : ' · liegt gerade auf einem Knoten anderer Stufe')
                + '</span></div><div class="control">'
                + (technik.passtZurStufe ? '' : '<span class="badge danger">Umzug fällig</span>')
                + '</div></div>'
                + '</div></div>';
        }
        html += playerCard();
    ...
    // Embed-Editor
    //
    // Ein Baustein fuer vier Module. Frueher hatte jedes seine eigenen drei
    // bis fuenf Felder und baute daraus sein Embed selbst - wer mehr wollte,
    // hatte Pech, und wer dasselbe Aussehen zweimal wollte, tippte alles
    // doppelt. Hier steht der volle Umfang dessen, was Discord kann, an einer
    // Stelle.
    // ------------------------------------------------------------------

    /** Leere Vorlage - dieselben Namen wie im EmbedVorlageDto. */
    function leeresEmbed() {
        return {
            id: "", name: "", inhalt: "",
            autorName: "", autorIconUrl: "", autorUrl: "",
            titel: "", titelUrl: "", beschreibung: "", farbe: "#78D1FF",
            thumbnailUrl: "", bildUrl: "", zusatzBilder: [], felder: [],
            fusszeile: "", fusszeileIconUrl: "", zeitstempel: false
        };
    }

    /**
     * @param basis      Pfad im Entwurf, etwa "embed." oder "panels.0.embed."
     * @param hinweis    modulspezifische Platzhalter, als HTML
     */
    function embedEditor(basis, hinweis) {
        var embed = getPath(draft, basis.replace(/\.$/, "")) || leeresEmbed();
        // Bestandsdaten haben die neuen Listen noch nicht.
        if (!embed.felder) { embed.felder = []; }
        if (!embed.zusatzBilder) { embed.zusatzBilder = []; }

        var vorlagen = (state.config && state.config.embedVorlagen) || [];
        var auswahl = '<option value="">— eigene Gestaltung —</option>';
        vorlagen.forEach(function (vorlage) {
            auswahl += '<option value="' + esc(vorlage.id) + '"'
                + (getPath(draft, basis.replace(/embed\.$/, "") + "embedVorlageId") === vorlage.id ? " selected" : "")
                + '>' + esc(vorlage.name || vorlage.id) + '</option>';
        });

        var html = '<details class="embed-editor"' + (embed.titel || embed.beschreibung || embed.inhalt ? " open" : "") + '>'
            + '<summary><b>Nachricht gestalten</b>'
            + '<span>Titel, Felder, Bilder, Fußzeile — der volle Umfang von Discord.</span></summary>'
            + '<div class="embed-editor-body">';

        if (vorlagen.length) {
            html += '<div class="field"><label>Vorlage</label>'
                + '<select class="select" data-p="' + basis.replace(/embed\.$/, "") + 'embedVorlageId">' + auswahl + '</select>'
                + '<div class="hint">Eine gemeinsame Vorlage gewinnt gegen alles darunter. So sehen mehrere Module gleich aus, ohne dass du es doppelt einträgst.</div></div>';
        }

        html += '<div class="hint">Bleibt alles leer, sieht die Nachricht aus wie bisher. '
            + (hinweis || "") + '</div>'

            + '<div class="field"><label>Text über dem Embed</label>'
            + '<textarea class="textarea" rows="2" data-p="' + basis + 'inhalt">' + esc(embed.inhalt) + '</textarea>'
            + '<div class="hint">Der einzige Ort, an dem Erwähnungen wirklich benachrichtigen — im Embed sehen sie nur so aus.</div></div>'

            + '<div class="embed-zeile">'
            + '<div class="field"><label>Autorzeile</label><input class="input" data-p="' + basis + 'autorName" value="' + esc(embed.autorName) + '" placeholder="klein über dem Titel"></div>'
            + '<div class="field"><label>Autor-Icon</label><input class="input mono" data-p="' + basis + 'autorIconUrl" value="' + esc(embed.autorIconUrl) + '" placeholder="https://…"></div>'
            + '</div>'

            + '<div class="embed-zeile">'
            + '<div class="field"><label>Titel</label><input class="input" data-p="' + basis + 'titel" value="' + esc(embed.titel) + '"></div>'
            + '<div class="field"><label>Titel verlinkt auf</label><input class="input mono" data-p="' + basis + 'titelUrl" value="' + esc(embed.titelUrl) + '" placeholder="https://…"></div>'
            + '</div>'

            + '<div class="field"><label>Beschreibung</label>'
            + '<textarea class="textarea" rows="4" data-p="' + basis + 'beschreibung">' + esc(embed.beschreibung) + '</textarea></div>'

            + '<div class="embed-zeile">'
            + '<div class="field"><label>Farbe</label><div class="input-group" style="max-width:240px">'
            + '<input type="color" class="input" data-p="' + basis + 'farbe" data-kind="color" value="' + esc(embed.farbe || "#78D1FF") + '">'
            + '<input class="input mono" data-p="' + basis + 'farbe" value="' + esc(embed.farbe || "#78D1FF") + '"></div></div>'
            + '<div class="field"><label>Fußzeile</label><input class="input" data-p="' + basis + 'fusszeile" value="' + esc(embed.fusszeile) + '"></div>'
            + '</div>'

            + '<div class="embed-zeile">'
            + '<div class="field"><label>Thumbnail (klein, rechts oben)</label><input class="input mono" data-p="' + basis + 'thumbnailUrl" value="' + esc(embed.thumbnailUrl) + '" placeholder="https://…"></div>'
            + '<div class="field"><label>Bild (groß, im Embed)</label><input class="input mono" data-p="' + basis + 'bildUrl" value="' + esc(embed.bildUrl) + '" placeholder="https://…"></div>'
            + '</div>'

            + switchRow(basis + "zeitstempel", "Uhrzeit anzeigen", "Discord schreibt „heute um …“ in die Fußzeile.");

        // Felder
        html += '<div class="field"><label>Felder</label><div class="repeat">';
        embed.felder.forEach(function (feld, i) {
            var fb = basis + "felder." + i + ".";
            html += '<div class="repeat-item"><span class="fields">'
                + '<input class="input" data-p="' + fb + 'name" value="' + esc(feld.name) + '" placeholder="Überschrift">'
                + '<input class="input" data-p="' + fb + 'wert" value="' + esc(feld.wert) + '" placeholder="Inhalt">'
                + '<label class="switch-inline"><input type="checkbox" data-p="' + fb + 'inline" data-kind="bool"'
                + (feld.inline === false ? "" : " checked") + '><span>nebeneinander</span></label>'
                + '</span>'
                + '<button class="icon-btn danger" data-remove="' + basis.replace(/\.$/, "") + '.felder|' + i + '" title="Entfernen">'
                + '<svg viewBox="0 0 24 24"><use href="#i-trash"/></svg></button></div>';
        });
        if (!embed.felder.length) {
            html += '<div class="repeat-empty">Keine Felder — die Beschreibung steht dann allein.</div>';
        }
        html += '</div><button class="btn btn-secondary btn-sm" style="margin-top:10px" data-add="'
            + basis.replace(/\.$/, "") + '.felder|embedFeld"><svg viewBox="0 0 24 24"><use href="#i-plus"/></svg>Feld hinzufügen</button>'
            + '<div class="hint">Bis zu 25. „Nebeneinander“ stellt bis zu drei in eine Reihe.</div></div>';

        // Zusatzbilder
        html += '<div class="field"><label>Weitere Bilder</label><div class="repeat">';
        embed.zusatzBilder.forEach(function (bild, i) {
            html += '<div class="repeat-item"><span class="fields">'
                + '<input class="input mono" data-p="' + basis + 'zusatzBilder.' + i + '" value="' + esc(bild) + '" placeholder="https://…">'
                + '</span>'
                + '<button class="icon-btn danger" data-remove="' + basis.replace(/\.$/, "") + '.zusatzBilder|' + i + '" title="Entfernen">'
                + '<svg viewBox="0 0 24 24"><use href="#i-trash"/></svg></button></div>';
        });
        if (!embed.zusatzBilder.length) {
            html += '<div class="repeat-empty">Keine weiteren Bilder.</div>';
        }
        html += '</div><button class="btn btn-secondary btn-sm" style="margin-top:10px" data-add="'
            + basis.replace(/\.$/, "") + '.zusatzBilder|embedBild"><svg viewBox="0 0 24 24"><use href="#i-plus"/></svg>Bild hinzufügen</button>'
            + '<div class="hint">Stehen <b>außerhalb</b> des Embeds, aber in derselben Nachricht — Discord stapelt sie zu einer Galerie. Bis zu neun.</div></div>';
    ...
    /**
     * Die gemeinsame Vorlagen-Bibliothek.
     *
     * <p>Bewusst kein Modul: die Vorlagen gehoeren keinem einzelnen, sondern
     * werden von mehreren benutzt. Deshalb auch ein eigener Speicherknopf
     * statt der gemeinsamen Leiste.
     */
    function renderVorlagen() {
        if (!state.vorlagenEntwurf) {
            state.vorlagenEntwurf = JSON.parse(JSON.stringify(
                (state.config && state.config.embedVorlagen) || []));
        }
        draft = { vorlagen: state.vorlagenEntwurf };
        draftModule = null;

        var html = head("vorlagen",
            "Einmal gestalten, in mehreren Modulen benutzen. Willkommen, Verify, Reaction-Roles und Tickets können jede dieser Vorlagen auswählen.");

        state.vorlagenEntwurf.forEach(function (vorlage, index) {
            html += '<div class="card"><h2>' + esc(vorlage.name || "Vorlage " + (index + 1))
                + '<span class="sub mono">' + esc(vorlage.id || "wird beim Speichern vergeben") + '</span></h2>'
                + '<div class="card-body">'
                + '<div class="field"><label>Name</label>'
                + '<input class="input" data-p="vorlagen.' + index + '.name" value="' + esc(vorlage.name || "") + '" placeholder="z. B. Serverdesign"></div>'
                + embedEditor("vorlagen." + index + ".", "")
                + '<div class="row"><div class="label"><b>Vorlage entfernen</b>'
                + '<span>Module, die sie benutzen, fallen auf ihre eigene Gestaltung zurück.</span></div>'
                + '<div class="control"><button class="btn btn-danger btn-sm" data-remove="vorlagen|' + index + '">Entfernen</button></div></div>'
                + '</div></div>';
        });

        if (!state.vorlagenEntwurf.length) {
            html += '<div class="notice">Noch keine Vorlage. Ohne eine bleibt jedes Modul bei seiner eigenen Gestaltung — das ist völlig in Ordnung.</div>';
        }

        html += '<div class="card"><div class="card-body" style="display:flex;gap:10px;flex-wrap:wrap">'
            + '<button class="btn btn-secondary" data-add="vorlagen|embedVorlage">'
            + '<svg viewBox="0 0 24 24"><use href="#i-plus"/></svg>Vorlage hinzufügen</button>'
            + '<button class="btn btn-primary" data-action="save-vorlagen">Speichern</button>'
            + '</div></div>';
        return html;
    }
    ...
    function renderWelcome() {
        ...
        html += '<div id="modBody"' + (draft.enabled ? "" : ' class="dim"') + '>'
            ...
            + '</div></div>'
            + '<div class="card"><h2>Eigene Nachricht</h2><div class="card-body">'
            + embedEditor("embed.", "Platzhalter: <code>{user}</code>, <code>{name}</code>, <code>{server}</code>, <code>{mitglieder}</code>, <code>{avatar}</code>.")
            + '</div></div>'
    ...
    function renderVerify() {
        ...
        html += '<div id="modBody"' + (draft.enabled ? "" : ' class="dim"') + '>'
            ...
            + '</div></div>'
            + '<div class="card"><h2>Eigene Nachricht</h2><div class="card-body">'
            + embedEditor("embed.", "Platzhalter: <code>{server}</code>, <code>{mitglieder}</code>.")
            + '</div></div>'
            ...
            + roleMulti("removedRoleIds", "Rollen entziehen",
                "Wird nach erfolgreicher Verifizierung wieder abgenommen — typisch eine „Ungeprüft“-Rolle, die alles verbirgt.")
            + '</div></div>'
            + '</div>';
        return html;
    }
    ...
    function renderReactionRoles() {
        ...
        draft.panels.forEach(function (panel, panelIndex) {
            ...
            html += '<div class="card"><h2>Panel ' + (panelIndex + 1)
                ...
                + embedEditor(base + "embed.", "Zusätzlich: <code>{zuordnungen}</code> fügt die Liste aus Emoji und Rolle ein — dann musst du sie nicht von Hand pflegen.")
            ...
            (panel.entries || []).forEach(function (entry, entryIndex) {
                ...
                html += '<div class="repeat-item">'
                    ...
                    + '<div style="grid-column:1/-1">' + roleMulti(entryBase + "roleIds", "Rollen vergeben", "") + '</div>'
                    + '<div style="grid-column:1/-1">' + roleMulti(entryBase + "removedRoleIds", "Rollen entziehen",
                        "Wird beim Reagieren abgenommen und beim Zurücknehmen wieder vergeben — so schließen sich zwei Rollen gegenseitig aus.") + '</div>'
                    + '</span>'
    ...
    function renderTickets() {
        ...
        draft.panels.forEach(function (panel, panelIndex) {
            ...
            html += '<div class="card"><h2>' + esc(panel.title || "Panel " + (panelIndex + 1))
                ...
                + colorField(base + "accentColor", "Akzentfarbe")
                + embedEditor(base + "embed.", "Platzhalter: <code>{server}</code>, <code>{mitglieder}</code>.")
    ...
    /**
     * Die Platzhalter fuer den Kanalnamen, zum Anklicken.
     *
     * <p>Sie standen bisher nirgends - man musste raten oder im Quelltext
     * nachsehen. Die Liste ist wortgleich mit
     * VoiceChannelModuleListener.buildChannelName(); wer dort etwas ergaenzt,
     * ergaenzt es hier mit.
     */
    function platzhalterHilfe() {
        var gruppen = [
            ...
            ["Wo", [
                ["{server}", "Servername"],
                ["{kanal}", "Auslöser-Kanal"],
                ["{kategorie}", "Kategorie"]
            ]],
            ["Wie viele", [
                ["{nummer}", "kleinste freie Nummer"],
        ...
        var html = '<div class="hint" style="margin-top:14px"><b>Platzhalter für den Namen</b>'
            + ' — anklicken kopiert sie in die Zwischenablage.</div>'
            + '<div class="platzhalter">';
        gruppen.forEach(function (gruppe) {
            html += '<div class="platzhalter-gruppe"><span class="platzhalter-titel">' + esc(gruppe[0]) + '</span>';
            gruppe[1].forEach(function (eintrag) {
                html += '<button type="button" class="marke" data-platzhalter="' + esc(eintrag[0]) + '" '
                    + 'title="' + esc(eintrag[1]) + '">' + esc(eintrag[0]) + '</button>';
            });
            html += '</div>';
        });
        html += '</div>'
            + '<div class="hint" style="margin-top:8px">{counter} und {count} gibt es weiterhin — sie bedeuten dasselbe wie {nummer}. '
            + 'Neu daran: eine gelöschte Nummer wird wiederverwendet. Wer #1 schließt, bekommt beim nächsten Kanal wieder #1 '
            + 'statt immer weiter hochzuzählen.</div>';
        return html;
    }

    /**
     * Der eigene Kurzlink auf die Server-Einladung.
     *
     * <p>Bewusst kein Modul im Sinne der anderen Seiten: er haengt nicht am
     * Bot, sondern an der Adresse der Weboberflaeche, und speichert ueber
     * einen eigenen Weg. Deshalb auch eine eigene Speicherleiste statt der
     * gemeinsamen.
     */
    function renderInviteLink() {
        var daten = state.invite || {};

        var html = head("invitelink",
            "Ein kurzer, selbst gewählter Link auf die Einladung deines Servers — für Signaturen, Flyer oder fremde Chats.");

        html += '<div class="card"><h2>Link</h2><div class="card-body">'
            + '<div class="row"><div class="label"><b>Aktiv</b>'
            + '<span>Ist der Link aus, läuft er ins Leere.</span></div>'
            + '<div class="control"><label class="switch"><input type="checkbox" id="invEnabled"'
            + (daten.enabled ? " checked" : "") + '><span></span></label></div></div>'
            + '<div class="field"><label>Kurzname</label>'
            + '<div class="input-group"><span class="prefix">' + esc(basisAdresse()) + '/invite/</span>'
            + '<input class="input mono" id="invSlug" value="' + esc(daten.slug || "") + '" placeholder="meinserver" autocomplete="off"></div>'
            + '<div class="hint">2 bis 32 Zeichen: Kleinbuchstaben, Ziffern und Bindestriche. '
            + 'Der Name gilt für alle Server gemeinsam — ist er vergeben, sagen wir es beim Speichern.</div></div>'
            + '<div class="field"><label>Ziel</label>'
            + '<input class="input mono" id="invTarget" value="' + esc(daten.targetUrl || "") + '" placeholder="https://discord.gg/…" autocomplete="off">'
            + '<div class="hint">Die Einladung aus Discord: Servereinstellungen → Einladungen. '
            + 'Nimm eine, die nicht abläuft — sonst zeigt dein Kurzlink irgendwann ins Nichts.</div></div>'
            + '</div></div>';

        if (daten.publicUrl) {
            html += '<div class="card"><h2>Fertig</h2><div class="card-body">'
                + '<div class="row"><div class="label"><b>Dein Link</b>'
                + '<span class="mono">' + esc(daten.publicUrl) + '</span></div>'
                + '<div class="control"><button class="btn btn-secondary btn-sm" data-copy="' + esc(daten.publicUrl) + '">Kopieren</button></div></div>'
                + '<div class="row"><div class="label"><b>Aufrufe</b>'
                + '<span>Wie oft der Link seit dem Anlegen benutzt wurde.</span></div>'
                + '<div class="control"><span class="pill' + (daten.clicks ? " on" : "") + '">' + (daten.clicks || 0) + '</span></div></div>'
                + '</div></div>';
        }

        html += '<div class="card"><div class="card-body">'
            + '<button class="btn btn-primary" data-action="save-invite">Speichern</button>'
            + '</div></div>';
        return html;
    }

    /** Die oeffentliche Adresse ohne Schema - nur fuer die Anzeige vor dem Feld. */
    function basisAdresse() {
        var roh = (state.invite && state.invite.publicUrl) || window.location.origin;
        return String(roh).replace(/^https?:\/\//, "").split("/")[0];
    }
    ...
    function renderJtc() {
        ...
        html += '</div><button class="btn btn-secondary btn-sm" style="margin-top:12px" data-add="entries|jtcEntry">'
            + '<svg viewBox="0 0 24 24"><use href="#i-plus"/></svg>Auslöser hinzufügen</button>'
            + platzhalterHilfe()
            + '</div></div>'
    ...
    var RENDERERS = {
        ...
        invitelink: renderInviteLink, vorlagen: renderVorlagen
    };
    ...
    /**
     * Liegt der Fokus gerade in einem Eingabefeld dieser Seite?
     *
     * Dann darf nicht neu gebaut werden - das Feld waere weg, mitten im Wort.
     * Die Auffrischung wird stattdessen gemerkt und nachgeholt, sobald das
     * Feld den Fokus verliert.
     */
    function eingabeAktiv() {
        var element = document.activeElement;
        if (!element || !page.contains(element)) { return false; }
        if (element.isContentEditable) { return true; }
        var tag = (element.tagName || "").toLowerCase();
        return tag === "input" || tag === "textarea" || tag === "select";
    }

    function fluechtigesSichern() {
        var werte = {};
        FLUECHTIG.forEach(function (id) {
            var feld = document.getElementById(id);
            if (feld) {
                werte[id] = { wert: feld.value, aktiv: document.activeElement === feld };
            }
        });
        return werte;
    }

    function fluechtigesZurueck(werte) {
        Object.keys(werte).forEach(function (id) {
            var gemerkt = werte[id];
            var feld = document.getElementById(id);
            if (!feld || !gemerkt.wert) { return; }
            if (feld.tagName.toLowerCase() === "select") {
                // Der Kanal kann inzwischen verschwunden sein - dann bleibt
                // die frisch gerenderte Auswahl stehen.
                var passt = Array.prototype.some.call(feld.options, function (option) {
                    return option.value === gemerkt.wert;
                });
                if (!passt) { return; }
            }
            feld.value = gemerkt.wert;
            if (gemerkt.aktiv) {
                feld.focus();
                try {
                    feld.setSelectionRange(feld.value.length, feld.value.length);
                } catch (ignored) {
                    // Bei <select> und type=color gibt es keinen Cursor.
                }
            }
        });
    }
    ...
    function show(name) {
        ...
        var seitenwechsel = vorherigeSeite !== name;
        vorherigeSeite = name;
        neuaufbauOffen = false;

        var gesichert = fluechtigesSichern();
        page.innerHTML = RENDERERS[name]();
        fluechtigesZurueck(gesichert);

        // Nur beim echten Wechsel nach oben springen. Bei der Auffrischung
        // alle fuenf Sekunden waere das Gescrolle sonst nicht auszuhalten.
        if (seitenwechsel) {
    ...
    /**
     * Blendet Bereiche aus, die fuer diesen Server nicht freigeschaltet sind.
     *
     * Das ist Kosmetik - gesperrt wird im Server. Aber ein Menuepunkt, der nur
     * zu einer Absage fuehrt, sieht nach Fehler aus statt nach Absicht.
     */
    function freigabenAnwenden() {
        var frei = (state.config && state.config.entitlements) || {};
        var abhaengig = { llm: "llmChat" };

        Object.keys(abhaengig).forEach(function (seite) {
            var erlaubt = frei[abhaengig[seite]] === true;
            var knopf = document.querySelector('[data-page="' + seite + '"]');
            if (knopf) {
                knopf.hidden = !erlaubt;
                // Die Gruppenueberschrift verschwindet mit, sonst bleibt eine
                // leere "Assistent"-Zeile stehen.
                var gruppe = knopf.closest(".nav-group");
                if (gruppe) {
                    var sichtbar = Array.prototype.filter.call(
                        gruppe.querySelectorAll(".nav-link"),
                        function (element) { return !element.hidden; });
                    gruppe.hidden = sichtbar.length === 0;
                }
            }
            // Steht der Nutzer gerade auf einer gesperrten Seite, zurueck.
            if (!erlaubt && state.page === seite) {
                state.page = "overview";
            }
        });
    }
    ...
    /**
     * Adresse und Zustand zusammenhalten.
     *
     * <p>Bisher lebte die Serverauswahl nur im Speicher. Nach F5 landete man
     * wieder in der Auswahl - mitten in der Arbeit, ohne Grund. Jetzt steht sie
     * im Adressteil hinter der Raute. Das ueberlebt nicht nur das Neuladen,
     * sondern gibt auch Vor und Zurueck einen Sinn und macht Links teilbar.
     */
    function adresseSchreiben() {
        if (!state.guildId) { return; }
        var neueAdresse = "#/server/" + state.guildId + "/" + (state.page || "overview");
        if (window.location.hash !== neueAdresse) {
            adresseSelbstGesetzt = true;
            window.location.hash = neueAdresse;
        }
    }

    /** Liefert {guildId, page} aus der Adresse - oder null, wenn nichts da steht. */
    function adresseLesen() {
        var roh = (window.location.hash || "").replace(/^#\/?/, "");
        var teile = roh.split("/").filter(Boolean);
        if (teile.length < 2 || teile[0] !== "server") { return null; }
        return { guildId: teile[1], page: teile[2] || "overview" };
    }

    async function selectGuild(guildId, startSeite) {
        ...
        try {
            var results = await Promise.all([
                ...
                api("GET", "/api/dashboard/radio/stations?guildId=" + encodeURIComponent(guildId)),
                // 403 fuer alle ohne Recht - dann bleibt der Technik-Block weg.
                api("GET", "/api/dashboard/guilds/" + guildId + "/stream").catch(function () { return null; }),
                api("GET", "/api/dashboard/guilds/" + guildId + "/invite").catch(function () { return null; })
            ]);
            state.config = results[0];
            state.player = results[1];
            state.voiceChannels = results[2];
            state.stations = results[3];
            state.stream = results[4];
            state.invite = results[5];
        } catch (error) {
        ...
        freigabenAnwenden();
        refreshBadges();
        // Gewuenschte Seite erst hier setzen: freigabenAnwenden() darf vorher
        // noch eine gesperrte Seite wegnehmen.
        if (startSeite && PAGES[startSeite]) {
            state.page = startSeite;
        }
        show(state.page);
        adresseSchreiben();
    }
    ...
    async function refreshPlayer(force) {
        ...
        try {
            ...
            //
            // Vorsicht bei "nichts läuft": dann ist currentTrack null, und die
            // frühere Fassung wertete das als Unterschied. Ergebnis war ein
            // Neuaufbau bei jedem Tick - genau der Fall, in dem man das
            // Suchfeld benutzt und der getippte Text nach fünf Sekunden weg war.
            var beideLeer = Boolean(previous) && !player.currentTrack && !previous.currentTrack;
            var sameTrack = Boolean(previous)
                && previous.paused === player.paused
                && (previous.queue || []).length === (player.queue || []).length
                && Boolean(previous.playingRadio) === Boolean(player.playingRadio)
                && (beideLeer || (player.currentTrack && previous.currentTrack
            ...
            if (force || !sameTrack) {
                if (state.page === "player" || state.page === "queue" || state.page === "overview" || state.page === "radio") {
                    // Getippt wird gerade? Dann später - siehe eingabeAktiv().
                    if (!force && eingabeAktiv()) {
                        neuaufbauOffen = true;
                        return;
                    }
                    show(state.page);
        ...
        switch (moduleName) {
            case "welcome":
                return {
                    ...
                    embed: working.embed, embedVorlageId: working.embedVorlageId
                };
            case "verify":
                return {
                    enabled: working.enabled, publishChannelId: working.publishChannelId,
                    verifiedRoleIds: working.verifiedRoleIds, removedRoleIds: working.removedRoleIds,
                    embed: working.embed, embedVorlageId: working.embedVorlageId,
                    title: working.title,
    ...
    // Eine Auffrischung, die wegen einer laufenden Eingabe zurückgestellt
    // wurde, wird nachgeholt - aber erst, wenn der Fokus die Seite wirklich
    // verlassen hat, nicht beim Sprung ins nächste Feld.
    page.addEventListener("focusout", function () {
        window.setTimeout(function () {
            if (neuaufbauOffen && !eingabeAktiv()) {
                show(state.page);
            }
        }, 0);
    });
    ...
    page.addEventListener("click", function (event) {
        var kopieren = event.target.closest("[data-copy]");
        if (kopieren && navigator.clipboard) {
            navigator.clipboard.writeText(kopieren.dataset.copy).then(function () {
                toast("Link kopiert.");
            }, function () {
                toast("Kopieren hat nicht geklappt.", true);
            });
            return;
        }

        var marke = event.target.closest("[data-platzhalter]");
        if (!marke) { return; }
        var text = marke.dataset.platzhalter;
        // Bevorzugt in das Feld, an dem gerade gearbeitet wird - sonst nur
        // in die Zwischenablage. Ein Platzhalter, den man erst suchen muss,
        // ist keine Hilfe.
        var feld = page.querySelector('input[data-p$="nameTemplate"]:focus');
        if (feld) {
            var pos = feld.selectionStart === null ? feld.value.length : feld.selectionStart;
            feld.value = feld.value.slice(0, pos) + text + feld.value.slice(feld.selectionEnd);
            feld.dispatchEvent(new Event("input", { bubbles: true }));
            feld.focus();
            feld.setSelectionRange(pos + text.length, pos + text.length);
            return;
        }
        if (navigator.clipboard) {
            navigator.clipboard.writeText(text).then(function () {
                ...
                toast("Kopieren hat nicht geklappt — bitte von Hand tippen.", true);
            });
        }
    });
    ...
    function newEntry(kind, index) {
        switch (kind) {
            ...
            case "rrEntry":
                return { id: "", emoji: "", roleIds: [], removedRoleIds: [], label: "", description: "" };
            case "ticketPanel":
            ...
            case "embedVorlage":
                return Object.assign(leeresEmbed(), { name: "Neue Vorlage" });
            case "embedFeld":
                return { name: "", wert: "", inline: true };
            case "embedBild":
                return "";
            case "jtcEntry":
    ...
    async function runAction(name, element) {
        if (name === "save-vorlagen") {
            element.disabled = true;
            try {
                var antwortV = await api("POST", "/api/dashboard/guilds/" + state.guildId + "/embed-vorlagen",
                    state.vorlagenEntwurf || []);
                toast(antwortV.message);
                state.config = await api("GET", "/api/dashboard/guilds/" + state.guildId + "/config");
                state.vorlagenEntwurf = null;
                show(state.page);
            } catch (error) {
                toast(error.message, true);
            } finally {
        ...
        if (name === "save-invite") {
            element.disabled = true;
            try {
                var antwort = await api("POST", "/api/dashboard/guilds/" + state.guildId + "/invite", {
                    enabled: document.getElementById("invEnabled").checked,
                    slug: document.getElementById("invSlug").value.trim(),
                    targetUrl: document.getElementById("invTarget").value.trim()
                });
                toast(antwort.message);
                state.invite = await api("GET", "/api/dashboard/guilds/" + state.guildId + "/invite");
                show(state.page);
            } catch (error) {
                toast(error.message, true);
            } finally {
    ...
    // Zurueck und Vor im Browser. Eigene Adressaenderungen werden uebersprungen,
    // sonst laedt jeder Seitenwechsel den Server ein zweites Mal.
    window.addEventListener("hashchange", function () {
        if (adresseSelbstGesetzt) {
            adresseSelbstGesetzt = false;
            return;
        }
        var ziel = adresseLesen();
        if (!ziel) { return; }
        if (ziel.guildId !== state.guildId) {
            selectGuild(ziel.guildId, ziel.page);
        } else if (ziel.page !== state.page) {
            show(ziel.page);
        }
    });
    ...
    if (state.guilds.length) {
        var gemerkt = adresseLesen();
        // Nur oeffnen, was diesem Nutzer auch gehoert - eine fremde ID in der
        // Adresse darf nicht in eine Fehlermeldung laufen.
        var erlaubt = gemerkt && state.guilds.some(function (g) { return g.id === gemerkt.guildId; });
        if (erlaubt) {
            selectGuild(gemerkt.guildId, gemerkt.page);
        } else {
