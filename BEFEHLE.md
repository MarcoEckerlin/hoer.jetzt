...
**Ein Release für alle setzen** — im Adminbereich unter *Verbund* (getrennte
Installation, Port 8081), oder:
...
**Adminbereich nachträglich installieren** — er ist seit dem Umbau eine eigene
Anwendung und in der Grundinstallation nicht dabei:
...
Danach `127.0.0.1:8081`. `auto-update.sh` zieht ihn ab dann von selbst mit.

---

## Neue Oberfläche, alte Oberfläche

Seit v2026.08.15.12 liegt unter `/dashboard` die neue Oberfläche. Die
bisherige ist nicht weg, sondern unter **`/alt-dashboard`** erreichbar — als
Rückweg, falls in der neuen etwas fehlt. Beide arbeiten mit denselben Daten
und derselben Anmeldung; man kann zwischen ihnen hin und her wechseln, ohne
etwas zu verlieren.

Der Adminbereich ist in **beiden** nicht mehr enthalten. Er ist eine eigene
Installation auf Port 8081, siehe ANLEITUNG.md.
...
git push origin \
    ...
    refs/remotes/origin/admin:refs/heads/admin
git push origin --tags
```

Danach zur Gegenprobe `git ls-remote --heads <repo>` — es müssen **sechs**
