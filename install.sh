...
echo
info "Betriebsbereich: Uebersicht ueber alle Nodes und Audio-Knoten. Nur auf"
info "der Maschine noetig, von der aus verwaltet wird. Er laeuft auf einem"
info "eigenen Port und ist nur ueber ein Discord-Konto mit Adminrecht nutzbar."
read -r -p "    Betriebsbereich hier installieren? (j/n) [n]: " ADMIN_JA || true
HJ_ADMIN_PORT_HOST=""
if [[ "${ADMIN_JA:-n}" =~ ^[jJ] ]]; then
    if [[ -d "${ARBEIT}/admin/.git" ]]; then
        git -C "${ARBEIT}/admin" fetch -q origin admin
        git -C "${ARBEIT}/admin" reset -q --hard origin/admin
    else
        git clone -q -b admin --single-branch "$REPO" "${ARBEIT}/admin" \
            || fail "Zweig admin nicht erreichbar."
    fi
    frage HJ_ADMIN_PORT_HOST "Port fuer den Betriebsbereich" "8081"
    COMPOSE+=(-f "docker-compose.admin.yml")
    info "Erreichbar auf 127.0.0.1:${HJ_ADMIN_PORT_HOST} - nach aussen nur"
    info "ueber einen Reverse Proxy mit TLS oeffnen."
fi
...
HJ_ADMIN_PORT_HOST=${HJ_ADMIN_PORT_HOST:-8081}
HJ_ADMIN_BIND=127.0.0.1
ENV
chmod 600 "$UMGEBUNG"
