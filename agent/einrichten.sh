...
UMGEBUNG="/etc/hoerjetzt-agent.env"
...
HJ_NODE_NAME="${HJ_NODE_NAME:-$HJ_NODE_CONTAINER}"
...
sed "s#/opt/hoerjetzt-node#${KNOTEN_DIR}#g" "${HIER}/hoerjetzt-agent.service" \
    > /etc/systemd/system/hoerjetzt-agent.service

systemctl daemon-reload
systemctl enable --now hoerjetzt-agent >/dev/null 2>&1 || fail "Agent laesst sich nicht starten."
systemctl restart hoerjetzt-agent

sleep 2
if curl -fsS -m 5 "http://127.0.0.1:${HJ_AGENT_PORT}/gesund" >/dev/null 2>&1; then
    info "Agent laeuft auf ${HJ_AGENT_URL}"
else
    warn "Agent antwortet noch nicht: journalctl -u hoerjetzt-agent -n 30"
fi
