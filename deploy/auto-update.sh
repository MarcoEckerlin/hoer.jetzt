...
# Stand von core, ai-radio, lavalink, web und (falls installiert) admin dazugehoert - die Zweige haben keine
...
for zweig in main core ai-radio lavalink web admin; do
...
# Der Betriebsbereich ist ein Zusatz. Er gilt auf dieser Maschine als
# installiert, wenn sein Arbeitsverzeichnis existiert - dann muss die
# Zusatzdatei bei jedem Build und jedem Start mitgegeben werden. Ohne das
# wuerde "docker compose up -d" den laufenden admin-Container fuer verwaist
# halten und stillschweigend stehen lassen, waehrend alles andere neu startet.
COMPOSE=(-f "$COMPOSE_DATEI")
if [[ -d "${ARBEIT}/admin/.git" ]]; then
    COMPOSE+=(-f "docker-compose.admin.yml")
fi
...
for zweig in core ai-radio lavalink web admin; do
    # admin ist der einzige freiwillige Zweig: fehlt er hier, ist der
    # Betriebsbereich auf dieser Maschine schlicht nicht installiert. Das ist
    # der Normalfall und kein Fehler.
    if [[ "$zweig" == "admin" && ! -d "${ARBEIT}/admin/.git" ]]; then
        continue
    fi
