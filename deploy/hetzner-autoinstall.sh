...
for zweig in "${ZWEIGE[@]}"; do
    ...
    if [[ -d "${ziel}/.git" ]]; then
        git -C "$ziel" fetch -q origin "$zweig" && git -C "$ziel" reset -q --hard "origin/${zweig}"
        info "${zweig}: aktualisiert"
    else
        git clone -q -b "$zweig" --single-branch "$REPO" "$ziel" \
            || fail "Zweig ${zweig} nicht erreichbar. Privates Repository? Dann erst zugang-einrichten.sh."
        info "${zweig}: geholt"
    fi
done
