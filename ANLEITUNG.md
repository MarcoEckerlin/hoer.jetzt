...
3. Einmal `/admin` aufrufen — wer die Anwendung besitzt, wird automatisch
   Eigentümer. Vorher kommt niemand hinein. Der Adminbereich liegt in der Weboberfläche,
   erreichbar über das Benutzermenü unten links.
...
Er liegt in der Weboberfläche, erreichbar über das Benutzermenü unten links —
aber nur für Bot-Administratoren. Wer keiner ist, sieht den Eintrag nicht; wer
die Adresse trotzdem eintippt, bekommt vom Bot bei jedem einzelnen Aufruf eine
Absage. Das Ausblenden ist Bequemlichkeit, der Schutz liegt im Bot.

Er war zwischenzeitlich eine getrennte Installation mit eigenem Abbild und
eigenem Port. Sicherheitstechnisch war das sauberer, im Alltag aber
umständlich: zweiter Container, zweiter Reverse-Proxy-Eintrag, und die
Anmeldung musste über denselben Ursprung laufen. Seit v2026.08.16.7 ist er
wieder Teil der Weboberfläche.

| Seite | Wofür |
|---|---|
| **Verbund** | Alle Nodes mit Shards, Release und letzter Meldung. Hier wird das Ziel für den ganzen Verbund gesetzt. |
| **Audio-Knoten** | Auslastung je Knoten, Serverbelegung, *Neu verbinden*, Knotentabelle neu einlesen, Server neu verteilen. |
| **Einstellungen** | Die bisherige Verwaltungsoberfläche von core — Knoten eintragen, Bot-Admins, Freigaben. |

Wer beim ersten Mal hineinkommt: `/admin` einmal aufrufen. Wem die Anwendung
bei Discord gehört, wird dabei automatisch Eigentümer — vorher kommt niemand
hinein, auch kein Serverinhaber.
